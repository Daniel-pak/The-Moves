...
.service("UserService", ['$http', '$location', 'TokenService', '$localStorage', function ($http, $location, TokenService, $localStorage) {
    ...
    this.addEvent = function (eventId) {
        ...
        return $http.put(`http://localhost:9000/api/user-info/${$localStorage.user._id}`, {
    ...
    this.editUser = function (user) {
        ...
        return $http.put(`http://localhost:9000/api/user-info/${$localStorage.user._id}`, editedUser)
    }

    this.getMyEvents = function () {
        return $http.get(`http://localhost:9000/api/user-info/myEvents/${$localStorage.user._id}`)
    }
    ...
    this.getPostedEvents = function () {
        return $http.get(`http://localhost:9000/api/user-info/postedEvents/${$localStorage.user._id}`)
    }
            }])